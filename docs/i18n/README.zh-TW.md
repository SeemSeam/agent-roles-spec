...
- **版本**: `0.1.0`
- **等級**: `preview`
- **用途**: 建立和審計符合規範的 Agent Roles，並輸出可執行的合規與優化發現。
- **適合場景**: 起草新 Role、審計 Role source、打磨 memory、skills 和 prompts，以及檢查 catalog readiness。
- **包含內容**: Role authoring memory、角色建立與審計 skill、可複用 audit prompt、驗證說明和 host adapter display metadata。
- **Adapters**: CCB、Claude Code、Codex、HIVE。
