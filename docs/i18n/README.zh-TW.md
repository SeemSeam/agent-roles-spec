...
- **版本**: `0.2.0`
- **等級**: `preview`
- **用途**: 建立和審計符合規範的 Agent Roles，並在有價值時進行聯網的 skill 構造研究。
- **適合場景**: 起草新 Role、審計 Role source、打磨 memory、skills 和 prompts、檢查 catalog readiness，以及研究 skill 構造工具和技巧。
- **技能研究**: 使用受限的公開網路研究，優先參考官方文件和維護中的範例，並在研究影響設計時記錄來源。
- **包含內容**: Role authoring memory、角色建立與審計 skill、可複用建立/審計 prompts、skill 構造研究參考、驗證說明和 host adapter display metadata。
- **Adapters**: CCB、Claude Code、Codex、HIVE。
