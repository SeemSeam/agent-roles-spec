...
- **Версия**: `0.5.0`
- **Уровень**: `stable`
- **Назначение**: Проверяет архитектурный drift, границы, топологию зависимостей, решения, влияние изменений и компромиссы надёжности без зависимости от одного инструмента.
- **Лучше всего для**: циклов и направления зависимостей, границ модулей, fitness functions, drift ADR, радиуса миграции и надёжности распределённых систем.
- **Содержимое**: инструкции Role, focused skills для топологии, границ, fitness, решений, влияния и надёжности, опциональные источники evidence, evaluation suite, prompt, документация инструментов, plugins и host adapters.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
- **Установка**: `agent-roles add agentroles.archi`
- **Обновление**: `agent-roles update agentroles.archi`
...
- **Версия**: `0.2.2`
- **Уровень**: `preview`
- **Назначение**: Создаёт, исследует, blueprint-ит, ingest-ит и аудирует spec-compliant Agent Roles с evidence-backed gates.
- **Лучше всего для**: подготовки новых Roles, аудита Role source, research briefs, candidate scorecards, blueprint gates, проверки catalog readiness, а также исследования skill construction tools и techniques.
- **Исследование skills**: использует ограниченный публичный web research для актуальных skill construction tools и techniques, отдавая приоритет официальным docs и поддерживаемым examples.
- **Содержимое**: Role authoring memory, skills для создания/аудита, source-ingest, research, candidate-score и blueprint, переиспользуемые prompts, skill construction research reference, artifact templates, preview schemas, локальный inventory script, validation notes и host adapter display metadata.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
- **Установка**: `agent-roles install mother`
- **Обновление**: `agent-roles update mother`
- **Источник**: [`roles/mother`](../../roles/mother/)
