...
- **Version**: `0.2.2`
- **수준**: `preview`
- **Purpose**: spec-compliant Agent Roles를 만들고 research, blueprint, ingest, audit하며 evidence-backed gates로 제약합니다.
- **Best for**: 새 Roles 초안 작성, Role source audit, research briefs, candidate scorecards, blueprint gates, catalog readiness 확인, skill construction tools/techniques 조사.
- **스킬 조사**: 공식 docs와 maintained examples를 우선해 현재 skill construction tools/techniques를 제한된 공개 웹 연구로 확인합니다.
- **Contents**: Role authoring memory, creation/audit, source-ingest, research, candidate-score, blueprint skills, 재사용 가능한 prompts, skill construction research reference, artifact templates, preview schemas, local inventory script, validation notes, host adapter display metadata.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
