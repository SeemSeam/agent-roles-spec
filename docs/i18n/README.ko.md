...
- **Version**: `0.2.0`
- **수준**: `preview`
- **Purpose**: spec-compliant Agent Roles를 만들고 audit하며, 유용할 때 skill construction을 위한 web-backed research도 포함합니다.
- **Best for**: 새 Roles 초안 작성, Role source audit, memory/skills/prompts 개선, catalog readiness 확인, skill construction tools/techniques 조사.
- **스킬 조사**: 공식 docs와 maintained examples를 우선해 현재 skill construction tools/techniques를 제한된 공개 웹 연구로 확인합니다.
- **Contents**: Role authoring memory, Role creation/audit skill, 재사용 가능한 creation/audit prompts, skill construction research reference, validation notes, host adapter display metadata.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
