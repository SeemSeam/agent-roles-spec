...
- **版本**: `0.1.0`
- **等级**: `preview`
- **用途**: 创建和审计符合规范的 Agent Roles，并输出可执行的合规与优化发现。
- **适合场景**: 起草新 Role、审计 Role source、打磨 memory、skills 和 prompts，以及检查 catalog readiness。
- **包含内容**: Role authoring memory、角色创建与审计 skill、可复用 audit prompt、验证说明和 host adapter display metadata。
- **Adapters**: CCB、Claude Code、Codex、HIVE。
