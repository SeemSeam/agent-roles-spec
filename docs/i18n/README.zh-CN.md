...
- **版本**: `0.2.0`
- **等级**: `preview`
- **用途**: 创建和审计符合规范的 Agent Roles，并在有价值时进行联网的 skill 构造研究。
- **适合场景**: 起草新 Role、审计 Role source、打磨 memory、skills 和 prompts、检查 catalog readiness，以及研究 skill 构造工具和技巧。
- **技能研究**: 使用受限的公开网络研究，优先参考官方文档和维护中的示例，并在研究影响设计时记录来源。
- **包含内容**: Role authoring memory、角色创建与审计 skill、可复用创建/审计 prompts、skill 构造研究参考、验证说明和 host adapter display metadata。
- **Adapters**: CCB、Claude Code、Codex、HIVE。
