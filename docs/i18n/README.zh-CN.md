...
每个 Role Definition 都携带包版本元数据。`version` 是必需字段；发布到
catalog 的 Role 还应包含 `created_at` 和 `updated_at` 时间戳，方便用户和
Host 在安装、更新或挂载前比较 Role 修订。
...
npm 包不会内置可安装的 `roles/` catalog；请用 `agent-roles list` 从已配置
catalog 发现可用 Roles，再用 `agent-roles install <role>` 按需安装。`archi`
这类短名 alias 会解析到 `agentroles.archi` 这样的 canonical catalog ID。
...
JSON 输出会包含 Role `version`、digest，以及源 Role 提供的修订时间戳。
