...
- **Phiên bản**: `0.1.0`
- **Cấp độ**: `preview`
- **Mục đích**: Tạo và audit Agent Roles đúng spec, với findings có thể hành động về compliance và optimization.
- **Phù hợp cho**: phác thảo Roles mới, audit Role source, tinh chỉnh memory, skills và prompts, và kiểm tra catalog readiness.
- **Nội dung**: Role authoring memory, skill tạo/audit Role, audit prompt tái sử dụng, validation notes và host adapter display metadata.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
