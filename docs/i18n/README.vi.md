...
- **Phiên bản**: `0.2.2`
- **Cấp độ**: `preview`
- **Mục đích**: Tạo, nghiên cứu, lập blueprint, ingest và audit Agent Roles đúng spec với các gate dựa trên evidence.
- **Phù hợp cho**: phác thảo Roles mới, audit Role source, research briefs, candidate scorecards, blueprint gates, kiểm tra catalog readiness, và nghiên cứu tools/techniques xây dựng skill.
...
- **Nội dung**: Role authoring memory, skills tạo/audit, source-ingest, research, candidate-score và blueprint, prompts tái sử dụng, skill construction research reference, artifact templates, preview schemas, script inventory cục bộ, validation notes và host adapter display metadata.
- **Adapters**: CCB, Claude Code, Codex, HIVE.
